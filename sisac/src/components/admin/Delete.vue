    ...
    <div class="delete">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group bottom-margin">
          <label>Digite o Cpf do usuario que deseja Deletar</label>
          <input type="text" class="form-control" v-model="cpf" placeholder="CPF do USER">
        </div>

        <div class="lower-button">
          <button class="btn btn-primary btn-block bottom-margin-smaller" type="submit">Deletar</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      async handleSubmit(){
        axios.delete('api/user/' + this.cpf)
        .then(() => {
          this.showAlert = true;
          this.error = null;
